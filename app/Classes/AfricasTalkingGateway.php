...
#[AllowDynamicProperties] class AfricasTalkingGateway
{
    protected $_username;
    protected $_apiKey;
    ...
    private function executeJsonPost (): void
    {
        $ch = curl_init();
    ...
    private function getVoiceHost(): string
    {
        return ($this->_environment == 'sandbox') ? 'https://voice.sandbox.africastalking.com' : 'https://voice.africastalking.com';
    }

    private function getSendSmsUrl($extension_ = ""): string
    {
        return $this->getApiHost().'/version1/messaging'.$extension_;
    }

    private function getVoiceUrl(): string
    {
        return $this->getVoiceHost();
    }

    private function getUserDataUrl($extension_): string
    {
        return $this->getApiHost().'/version1/user'.$extension_;
    }

    private function getSubscriptionUrl($extension_): string
    {
        return $this->getApiHost().'/version1/subscription'.$extension_;
    }

    private function getAirtimeUrl($extension_): string
    {
        return $this->getApiHost().'/version1/airtime'.$extension_;
    }

    private function getBankPaymentCheckoutChargeUrl(): string
    {
        return $this->getPaymentHost().'/bank/checkout/charge';
    }

    private function getBankPaymentCheckoutValidationUrl(): string
    {
        return $this->getPaymentHost().'/bank/checkout/validate';
    }

    private function getBankPaymentTransferUrl(): string
    {
        return $this->getPaymentHost().'/bank/transfer';
    }

    private function getCardPaymentCheckoutChargeUrl(): string
    {
        return $this->getPaymentHost().'/card/checkout/charge';
    }

    private function getCardPaymentCheckoutValidationUrl(): string
    {
        return $this->getPaymentHost().'/card/checkout/validate';
    }

    private function getMobilePaymentCheckoutUrl(): string
    {
        return $this->getPaymentHost().'/mobile/checkout/request';
    }

    private function getMobilePaymentB2CUrl(): string
    {
        return $this->getPaymentHost().'/mobile/b2c/request';
    }

    private function getMobilePaymentB2BUrl(): string
    {
